...
int statement (void) {
    ...
    if (lookahead->tag == READ) {
        ...
        ok1 = match(WRITE);
        if (ok1 == true && lookahead->tag != ERROR){
            if ( lookahead->tag == STRING ) {
                ...
                ok1 = match(STRING);
                    if (gen_string != NULL ) {
                        ...
                        ok2 = match(SEMICOLON);
                        if (ok2 != true) {
                            printf("[ERRO] Eh esperado um ';' no final de write -> EX: WRITE STRING;\n");
                        }
                    }else{
                        printf("[ERRO] Não foi possivel inserir na tabela de strings\n");
                        return false;
                    }
                ...
                if (search_symbol != NULL) {
                    ...
                    if(match(SEMICOLON))
                        ...
                        printf("[ERRO] Eh esperado um ';' no final de write -> EX: WRITE ID; \n");
                } else {
                    printf("[ERRO] Simbolo desconhecido (Variavel nao declarada): %s\n", lexeme_of_id);
                    return false;
                }
            ...
            if(match(ERROR)){
                 printf("[ERRO] Tamanho da string eh maior que a capacidade do buffer ou fechamento de aspas duplas não identificada.\n");
                return false;
            }else{
